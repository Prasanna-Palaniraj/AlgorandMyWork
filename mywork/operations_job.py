...
from .job_contracts import approval_program, clear_state_program
from .util import (
    waitForTransaction,
...
def createJobApp(
    ...
    globalSchema = transaction.StateSchema(num_uints=11, num_byte_slices=8)
    localSchema = transaction.StateSchema(num_uints=0, num_byte_slices=0)

    app_args = [
        encoding.decode_address(poster),
        posttitle,
        postid.to_bytes(8, "big"),
        compensation.to_bytes(8, "big"),
        skill,
        skill_level.to_bytes(8, "big"),
    ]
...
def pullJobRequest(
    ...
    fundAppTxn = transaction.PaymentTxn(
        sender=poster.getAddress(),
        sp = suggestedparams,
        receiver= appAddress,
        amt= fundingAmount,
        note="first payment txn"
    )
    
    pullTxn = transaction.ApplicationCallTxn(
        sender=poster.getAddress(),
        index= appID,
        on_complete= transaction.OnComplete.NoOpOC,
        sp=suggestedparams,
        app_args=[b"pull",worker.getAddress()],
    )

    print(worker.getAddress())

    transaction.assign_group_id([fundAppTxn, pullTxn])

    signedFundAppTxn = fundAppTxn.sign(poster.getPrivateKey())
    signedPullTxn = pullTxn.sign(poster.getPrivateKey())

    client.send_transactions([signedFundAppTxn,signedPullTxn])

    response = waitForTransaction(client, signedPullTxn.get_txid())
    return response

def completeJobRequest(
    client: AlgodClient,
    worker: Account,
    poster: Account,
    appID: int,
    ...
    completeJobTxn = transaction.ApplicationCallTxn(
        index = appID,
        sender=poster.getAddress(),
        sp = suggestedParams,
        on_complete=transaction.OnComplete.NoOpOC,
    ...
    signedCompleteJobTxn = completeJobTxn.sign(poster.getPrivateKey())

    client.send_transaction(signedCompleteJobTxn)

    response = waitForTransaction(client, signedCompleteJobTxn.get_txid())
...
def transferFunds(
    ...
    payWorker = transaction.ApplicationDeleteTxn(
        sender=worker.getAddress(),
        sp = suggestedParams,
        index=appID,
        note="Job complete payment",
        #app_args=[workerAddress],
    )


    signedPayWorker = payWorker.sign(worker.getPrivateKey())


    client.send_transaction(signedPayWorker)

    response = waitForTransaction(client, signedPayWorker.get_txid())
